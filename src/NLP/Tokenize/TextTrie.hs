...
protectTerms terms cs =
  let realTerms = case cs of
                    Sensitive   -> terms
                    Insensitive -> map T.toLower terms
  in protectedTokens cs $ buildTrie realTerms
...
protectedTokens :: CaseSensitive -> PS.TrieSet M.Map Char -> RawToken -> [RawToken]
protectedTokens cs trie rawTok = reverse $ addLastToken $ T.foldl' fn emptyAcc $ text rawTok
  ...
  where
    ...
    fn acc char = let ch = case cs of
                             Sensitive   -> char
                             Insensitive -> Char.toLower char
                  in case M.lookup ch $ trieChildren1 (curTrie acc) of
      Nothing | inToken acc ->
                 case PS.member (reverse $ curText acc) trie of
                   ...
                   -- Nope! we are in an open token afterall
                   -- (just finished a *prefix* of something in the trie)
