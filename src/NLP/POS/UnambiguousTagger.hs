...
-- source of tags.
mkTagger :: POS t => Map Text t -> Maybe (POSTagger t) -> POSTagger t
mkTagger table mTgr = let
  litTagger = LT.mkTagger table LT.Sensitive mTgr

--  trainer :: POS t => [TaggedSentence t] -> IO (POSTagger t)
  trainer exs = do
...
train :: POS t => Map Text t -> [TaggedSentence t] -> Map Text t
train table exs = let

--  trainOnPair :: Map Text t -> POS t -> Map Text t
  trainOnPair t (Token txt, pos) = Map.alter (incorporate pos) txt t
...
--  incorporate :: t -> Maybe t -> Maybe t
  ...
  in foldl trainOnPair table $ concatMap tsToPairs exs
